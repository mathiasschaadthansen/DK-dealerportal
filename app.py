...
def vis_forventet_dato(forv, compact=False):
    if not forv or forv in ["None", ""]:
        return
    try:
        dage = (datetime.strptime(forv[:10], "%Y-%m-%d") - datetime.today()).days
        if compact:
            if dage < 0:    st.caption(f"📅 {forv[:10]} ⚠️ forsinket")
            elif dage == 0: st.caption("📅 **Ankommer i dag!**")
            else:           st.caption(f"📅 Forventet lager: {forv[:10]} ({dage}d)")
        else:
            if dage < 0:    st.warning(f"⚠️ Forventet på lager: {forv[:10]} — forsinket")
            elif dage == 0: st.success("📅 Ankommer **i dag**!")
            else:           st.info(f"📅 Forventet på lager: **{forv[:10]}** ({dage} dage)")
    except:
        if compact: st.caption(f"📅 {forv[:10]}")
        else:       st.info(f"📅 Forventet på lager: {forv[:10]}")
    ...
    if not st.session_state['cart']:
        ...
        for key, car in st.session_state['cart'].items():
            ...
            new_bid = st.number_input(
                ...
                min_value=0, value=int(current_bid), step=1000, key=f"bid_{key}"
            )
            st.session_state['cart'][key]['bid_price'] = new_bid
            total_bid_price += new_bid
            cart_text_lines.append(f"- {car['title']} (VIN: {car['vin']}) -> Mit bud: kr. {new_bid:,}".replace(',', '.'))
        st.write("---")
...
def show_car_details(row):
    ...
    if status_dk and status_dk.lower() not in ['aktiv', '']:
        if "vej" in status_dk.lower(): st.markdown(f"**Status:** ⏳ {status_dk}")
        else:                          st.markdown(f"**Status:** 🟢 {status_dk}")
    ...
    # ✅ FORVENTET LAGER DATO I DIALOG
    vis_forventet_dato(safe(row.get('forventet_lager_dato')), compact=False)
    ...
    with tab1:
        images = [u.strip() for u in safe(row.get('billede_url')).split(',') if u.strip().startswith('http')]
        if images:
            for img in images:
    ...
    if has_pdf or has_skader:
        with tab3:
            ...
            if has_skader:
                ...
                for img in [u.strip() for u in skade_string.split(',') if u.strip().startswith('http')]:
                    st.image(img, use_container_width=True)
                    st.write("---")
        ...
        for col, (_, row) in zip(cols, chunk.iterrows()):
            with col:
                with st.container(border=True):
                    ...
                    # ✅ FORVENTET LAGER DATO PÅ KORT
                    vis_forventet_dato(safe(row.get('forventet_lager_dato')), compact=True)
